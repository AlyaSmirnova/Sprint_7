...
# фикстура создает нового курьера и возвращает его данные
@pytest.fixture
def new_courier():
    with allure.step('Создаем курьера через фикстуру'):
        ...
        login_response = requests.post(f'{URL}/courier/login', data = login_payload)
        if login_response.status_code == 200:
            courier_id = login_response.json().get('id')
            requests.delete(f'{URL}/courier/{courier_id}')

def auth_courier(courier_data):
    with allure.step('Авторизация курьера'):
        return requests.post(f'{URL}/courier/login', data=courier_data)
